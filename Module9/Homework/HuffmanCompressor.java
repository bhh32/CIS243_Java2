...
public class HuffmanCompressor {
	...
	public HuffmanCompressor(String filename) {
		if (!filename.endsWith(".txt")) {
			throw new IllegalArgumentException("This compressor only works on text files!");
		}
		this.filename = filename.split(".txt")[0];
	}

	public void makeCode() throws IOException {
		System.out.println("I am about to make the Huffman code for " + filename + ".txt...");
		...
		System.out.println("\tI built up a frequency table of the " + "characters in your file.");
		System.out.println(
				"\tNow, I'm going to call your HuffmanCode(int[]) " + "constructor using that frequency table.");
		HuffmanCode t = new HuffmanCode(count);

		System.out.println("\tOkay!  Now, I am going to save the code (using " + "your save method) to the file "
				+ filename + ".code!");
	...
	public void compress(boolean debug) throws IOException {
		System.out.println("I am about to attempt to COMPRESS " + filename + ".txt:");
		/* We must make the code before we can compress... */
		this.makeCode();
		System.out
				.println("\tNow that I have the Huffman Code, I am going " + "to use the huffman code file created by");
		System.out.println("\tyour save() method to compress the contents " + "into " + filename + ".short!");
		...
		BitOutputStream output = new BitOutputStream(new PrintStream(this.filename + ".short"), debug);

		// Do the compression
		int n = input.read();
		while (n != -1) {
			if (codes[n] == null) {
				System.out.println("Your code file has no code for " + n + " (the character '" + (char) n + "')");
	...
	public void decompress(boolean printToConsole) throws IOException {
		System.out.println("I am about to attempt to DECOMPRESS " + filename + ".short:");

		System.out.println("\tTo do this, I must first read in the huffman " + "code used to compress the file.");
		System.out.println("\tI will use your HuffmanCode(Scanner) " + "constructor!");
		...
		if (!printToConsole) {
			System.out.println("\tNow, I will decompress the file using your " + "translate() method and save");
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("Welcome to the CS 143 Huffman Compressor!");
		System.out.println();
		...
		while (!done) {
			...
			while (choice == -1) {
				...
				System.out.println("\t(2) compress the file,");
				System.out.println("\t(3) decompress the file, or");
				System.out.println("\t(4) do a compression followed by a decompression, or");
				System.out.println("\t(5) switch to a new file, or");
			...
			boolean debug = false;
			if (choice == COMPRESS) {
				debug = prompt(console, "Would you like to debug the compressed file (y/n)? ");
			}

			boolean toConsole = false;
			if (choice == DECOMPRESS || choice == ROUND_TRIP) {
				toConsole = !prompt(console, "Would you like to print the result to a file (y/n)? ");
			}
	...
	private static String getFileName(Scanner console) {
		String filename;
		do {
			System.out.print("Which file would you like to work with (must be a txt file)? ");
