...
// Pianica class.  It does not test all of the functionality.
//
// The main method has an array of 37 frequency values that should be produced
// by a correctly working Pianica class.  The actual frequencies don't all end
// in 0.25, but this version of Note converts them to that form so that
// it is easier to understand the output produced by the programming.  In
// particular, it is easier to see if it is properly adding new values as notes
// are played.

public class TestPianica {
    ...
    public static void advance(Piano pianica, int tics) {
        for (int i = 0; i < tics; i++) {
            System.out.println("time " + pianica.time() + " sample = " + pianica.sample());
            pianica.tic();
        }
    }
}
